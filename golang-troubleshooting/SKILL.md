...
metadata:
  ...
  original-repo: https://github.com/samber/cc-skills-golang/tree/main/skills/golang-troubleshooting
  version: "1.1.2"
  adapted-by: openserbia
  adapted-changes: "Updated migration tooling references to goose, aligned cross-references with local skills"
---
...
"Migration failed or DB schema issue"
  → Check goose status: goose -dir migrations postgres "$DATABASE_URL" status
  → See golang-tooling skill for goose migration conventions
...
**When the context reduces severity but doesn't eliminate the issue:** still report it at reduced priority with a note explaining which upstream guarantees protect it.
...
- **[General Debugging Methodology](./references/methodology.md)** — The systematic 10-step process: define symptoms, isolate reproduction, form one hypothesis, test it, verify the root cause, and defend against regressions.

- **[Common Go Bugs](./references/common-go-bugs.md)** — The bugs that crash Go code: nil pointer dereferences, interface nil gotcha, variable shadowing, slice/map/defer/error/context pitfalls, race conditions, JSON unmarshaling surprises, unclosed resources.

- **[Test-Driven Debugging](./references/testing-debug.md)** — Why writing a failing test is the first step of debugging. Covers test isolation techniques, table-driven tests, useful `go test` flags (`-v`, `-run`, `-count=10` for flaky tests).

- **[Concurrency Debugging](./references/concurrency-debug.md)** — Race conditions, deadlocks, goroutine leaks. Race detector (`-race`), reading race detector output, detecting leaks with `goleak`.

- **[Performance Troubleshooting](./references/performance-debug.md)** — CPU profiling, memory analysis, lock contention, I/O blocking. How to read flamegraphs and measure improvement with benchmarks.

- **[pprof Reference](./references/pprof.md)** — Complete pprof manual. Enable endpoints, profile types, capturing locally and remotely, interactive analysis, interpreting flamegraphs.

- **[Diagnostic Tools](./references/diagnostic-tools.md)** — GODEBUG environment variables, Delve debugger, escape analysis, Go's execution tracer.

- **[Production Debugging](./references/production-debug.md)** — Debugging live production systems. Structured logs, pprof safely, capturing profiles, network debugging.

- **[Compilation Issues](./references/compilation.md)** — Build failures: module conflicts, CGO linking, version mismatch, build tags.

- **[Code Review Red Flags](./references/code-review-flags.md)** — Patterns to watch during review: unchecked errors, missing nil checks, concurrent map access, goroutines without exit, resource leaks.

## Attribution

Based on [samber/cc-skills-golang@golang-troubleshooting](https://github.com/samber/cc-skills-golang/tree/main/skills/golang-troubleshooting) (MIT License). Adapted for openserbia Go stack (goose migrations, slog logging, pgx).
